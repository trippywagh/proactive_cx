...
const COPY = {
  en: {
    ...
    safetyPill: '₹1,999 with NPCI · 100% recoverable',
    trackerLabels: ['Debited', 'Sending', "Receiver's Bank"],
  ...
  hi: {
    ...
    sub: 'तृप्तेश वाघ को',
    safetyPill: '₹1,999 NPCI के पास · पूरी तरह सुरक्षित',
    trackerLabels: ['कट गए', 'भेज रहे हैं', 'पाने वाले का बैंक'],
...
export default function PendingDebited() {
...
function SafetyNoteCard({ t }) {
  return (
    <section className="mx-4 mt-3 bg-white rounded-2xl border border-gray-100 p-3 relative overflow-hidden">
      <div className="absolute left-0 top-0 bottom-0 w-[3px] bg-emerald-500" />
      <div className="flex items-start gap-2.5">
        <div className="shrink-0 mt-0.5 w-7 h-7 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-600">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none">
            <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" fill="currentColor" opacity="0.18" />
            <path d="M9 12l2 2 4-4" stroke="currentColor" strokeWidth="2.2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </div>
        <p className="text-[11.5px] text-slate-600 leading-snug flex-1">{t.fallbackNote}</p>
      </div>
    </section>
  )
}
